...
目标：在 Shadowrocket 开启的状态下，懒猫微服依然能保持直连。

#### 1. 准备工作

- 确保已在 Shadowrocket 中导入并选择一个可用的节点配置文件。
    - *画面显示：Shadowrocket 主界面，已有一个名为 “default.conf” 的配置文件，并有多个节点列表。*

#### 2. 编辑配置文件 (以 “default.conf” 为例)

- 在 Shadowrocket 首页，点击顶部”全局路由”右侧的 **“配置”**。
- 找到您当前使用的配置文件（例如 “default.conf”）。
- **长按**该配置文件，在弹出菜单中选择 **“编辑配置”**。
    - *画面进入 “编辑配置 default.conf” 页面，包含 “通用”、“规则”、“Hosts” 等选项。*
    - **a. 进入”通用”设置**
        - 点击 **“通用”**。
        - 建议配置与视频演示保持一致：
            - 旁路系统：开启
            - 启用 IPv6：开启
            - 首选 IPv6：关闭 (视频中为关闭状态，但强调启用完整的IPv6支持)
            - 私有 IP 应答：开启
            - DNS 覆写：system
            - 备用 DNS：system
    - **b. 添加”跳过代理”规则**
        - 在”通用”设置页面，向下滚动找到 **“跳过代理”** (Skip Proxy) 部分。
        - 点击 **“+”** (以逗号分隔) 添加以下两条域名规则：
            - `.heiyu.space`
            - `.lazycat.cloud`
        - *画面显示：用户输入并添加了上述两条规则。*
    - **c. 添加”TUN 旁路路由”规则**
        - 继续向下滚动找到 **“TUN 旁路路由”** (TUN Bypass Route) 部分。
        - 点击 **“+” (IP-CIDR)** 添加以下两条 IP 地址/段规则：
            - `6.6.6.6/32`
            - `2000::6666/128`
        - **注意**：这两条规则务必正确填写，否则可能导致懒猫微服流量被错误代理。
        - *画面显示：用户输入并添加了上述两条规则。*
    - **d. 添加”总是真实 IP”规则**
        - 继续向下滚动找到 **“总是真实 IP”** (Always Real IP) 部分。
        - 点击 **“+”** (以逗号分隔) 添加以下两条域名规则（与”跳过代理”中的相同）：
            - `.heiyu.space`
            - `.lazycat.cloud`
        - *画面显示：用户输入并添加了上述两条规则。*
    - **e. 保存并应用配置**
        - 完成上述添加后，点击页面右上角的**保存按钮（通常为对勾图标）**。提示”成功”。
        - 返回 Shadowrocket 首页。
        - 确保”全局路由”模式选择为 **“配置”**。
        - 再次点击”全局路由”右侧的 **“配置”**，长按刚刚编辑的配置文件 (如 “default.conf”)，选择 **“使用配置”**，确保其被激活（通常有勾选标记或颜色高亮）。
