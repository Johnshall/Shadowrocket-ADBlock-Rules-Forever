...
为了确保用户在“独立思考”时不偏离本项目预设的正轨，特对声明中的核心词汇进行统一界定。请务必在遵循以下定义的前提下使用本项目：

    【独立思考】：指用户在接触外网信息时，能够迅速识别出任何与作者立场不符的观点，并自动将其归类为“洗脑”或“阴谋”的思维过程。其最终结果必须指向“屏障确实在保护我”。

    【误导性言论】：凡是揭露、核查或批判某些现状，且未能在本规则直连名单（国内信源）中找到对应肯定描述的言论。真相的标准不在于证据，而在于其是否符合“讲好故事”的基调。

    【美国阴谋】：一种元设定。不需要证据支撑，因为“善于运用舆论”本身就是最大的证据。如果某件事看起来不像是阴谋，那说明该阴谋已经通过大数据潜移默化地成功洗脑了你。

    【极端政治人士】：指那些试图通过逻辑推导、事实比对、要求注解或质疑本项目双标逻辑的用户。这类人因为独立思考过度，已对本项目构建的思想屏障产生威胁。

    【洗脑】：当用户对外网提供的逻辑、数据或事实产生“竟然有点道理”的认同感时，即判定为处于“被洗脑”状态。
